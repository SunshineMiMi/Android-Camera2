...
package net.lqtest.app.recorder;
...
public class Recorder implements OnCompletionListener, OnErrorListener {
	...
	public void startRecording(int outputfileformat, String extension, Context context) {
		Log.d("---startRecording---","ing");
		stop();
		
		Log.d("ProductTest AudioTest", "startRecording 1!");

		if (mSampleFile == null) {
			File sampleDir = new File("/data/");
			if (!sampleDir.canWrite())
				sampleDir = new File("/data/");
		...
		Log.d("Recorder", "---mSampleFile.getAbsolutePath() = "+mSampleFile.getAbsolutePath());
		...
		try {
			Runtime.getRuntime().exec("chmod 777 " + mSampleFile.getAbsolutePath());
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		Log.d("ProductTest AudioTest", "startRecording 2!");
	}
	...
	public void stopRecording() {
		...
		Log.d("ProductTest AudioTest", "stopRecording !");
		mSampleLength = (int) ((System.currentTimeMillis() - mSampleStart) / 1000);
		Log.d("Recorder", "mSampleLength == " + mSampleLength);
		mRecorder.stop();
		mRecorder.release();
		mRecorder = null;
		setState(IDLE_STATE);
